    ...
    <span class="top" @click="frameClickEvent('top')" title="置顶">
      ...
      >
        <path
          ...
          p-id="1189"
          :fill="windowIsOnTop ? '#555555' : '#ffffff'"
        ></path>
      </svg>
    </span>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const windowIsOnTop = computed(() => store.state.windowIsOnTop)
      const frameClickEvent = (e: string) => {
        ...
        if (e === 'top') {
          store.commit('SET_WINDOWISONTOP', !windowIsOnTop.value)
          win.setAlwaysOnTop(windowIsOnTop.value)
        }
      }
      return { windowIsOnTop, frameClickEvent }
    }
  })
</script>
